...
- [修复] backtest interval 校验：非法 interval（如 "2h"、""、"1D"）在 service/API/CLI 层严格拒绝（ValueError/HTTP 400/argparse error），不再静默走日线路径污染日线统计
- [修复] 分钟回测入场价与窗口起点对齐日线收盘语义：入场价改取 analysis_date 日线收盘（与日线路径一致）；分钟窗口起点改为 analysis_date + 1 day 00:00 UTC（日线 bar 收盘后第一根分钟 bar）
...
- [改进] 数字货币报告文案统一用计价币：crypto 市场指引要求价格/目标位/止损位用交易对计价币（如 USDT）表述，修复 LLM 正文把 USDT 价格误写成“元”的问题。
