...
**📌 当前暂定定位(2026-07-03,可升级)**:取 **(a) 自用 / 内部** 为默认档——批答未及时到位时取三档中**最保守**者(不对外分发信号 → 规避 A股证券投资咨询/投顾牌照全部对外敞口)。含义与约束:
- 信号仅用于**自用**;**不新增任何对外信号分发/推送渠道**;既有 notification 属自用推送,不构成对外分发。
- 该档为**可升级的临时决定**,非终局:后续可由人决策升级到 (b) 持牌 或 (c) 教育免责;**升级前**所有对外"明确买卖指令"分发路径保持关闭(与 §7 红线一致)。
- 各市场差异待定位升级时再细化:crypto / 港股通 / A股 的对外口径在升级为 (b)/(c) 时分别评估。
...
- **当前 (a) 自用/内部 定位下不对外分发**"明确 A股买卖指令"(§3);升级到持牌/教育免责并由人重开分发路径前保持关闭。
...
1. **§3 合规定位**已暂定 **(a) 自用 / 内部**(2026-07-03,最保守档,可升级)——若需对外分发需人决策升级到持牌/教育免责并重开分发路径。
