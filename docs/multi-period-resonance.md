...
| 月线根数有限 | `days=1825` 上限约对应 60 根月线，历史更长的标的无法展示更早月线（A股/港股/美股实测约 53 周线 / 61 月线） |
| crypto 多周期偏弱 | 加密日线源单次约返回 100 根日线（不随 `days` 放宽），故 crypto 周线≈15 / 月线≈4 根；月线远不足 MA20 的 20 根样本 → **crypto 共振实际恒为 neutral/none**。多周期对 crypto 仅作粗略图表参考；如需 crypto 共振，须后续为加密日线源加分页/扩历史（超出 M4-A 范围）。 |
