...
```
┌──────────────────────────────────────────┐
│ 行情头（随页面滚动，v1 非 sticky；股名 + 实时价格 + 涨跌幅）│
│ [☆ 自选]  [刷新分析]  [建告警]  [复制链接]         │
├──────────────────────────────────────────┤
...
- **行情头 v1 非 sticky**：行情头随页面滚动，不固定于视口顶部；spec 的 sticky 常驻效果为后续增强，v1 暂缓以规避移动端顶栏交互的未验证风险。
- **告警标的归一边缘**：工作台从看板/抽屉/报告进入时使用后端 canonical code，与告警页保持一致；但手敲非规范 URL（如 `/stock/aapl`）时，工作台按原始 code 创建/查询告警，可能与告警页经 `validateStockCode` 归一（`AAPL`）后的规则互不可见。建议始终从既有入口进入工作台，避免手敲非规范 code。
